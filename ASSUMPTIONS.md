...
- **Undo/redo cursor** moves only after a transaction step is confirmed (matching socket2 event, clients-j, or a successful `hyprctl dispatch` with no expected event). Timeout, non-zero hyprctl exit, or error text fails the batch, restores the cursor (it was never advanced), drops remaining scrub steps, and sets `lastStatus` to `failed:…`.
